...
def run_job(job_id: str, req: JobRequest, progress) -> JobResult:
    ...
    cam_floor, cam_render = [], []
    for i, c in enumerate(cams):
        def on_frame(done, total, i=i):
            progress("tracking", 0.45 + 0.25 * (i + done / max(total, 1)) / n)
        tracks, per_frame = track_from_dets(c.videoPath, cam_det[i]["dets"], cfg, on_frame)
        w, h = cam_wh[i]
    ...
    global_tracks, cam_to_global = fuse_tracks(cam_floor, cfg)
