...
class Config:
    ...
    R_MERGE_M = float(os.getenv("PRISM_R_MERGE_M", "1.0"))          # radius merge geometri ketat (m)
    MERGE_MIN_OVERLAP_SEC = float(os.getenv("PRISM_MERGE_OVERLAP", "0.5"))
    # Fusion pakai penampilan (ReID) antar-kamera: geometri longgar + gate appearance.
    WITH_APP_FUSION = os.getenv("PRISM_APP_FUSION", "1") != "0"
    R_MERGE_APP_M = float(os.getenv("PRISM_R_MERGE_APP_M", "2.5"))   # radius longgar bila appearance cocok
    APP_THRESH = float(os.getenv("PRISM_APP_THRESH", "0.7"))         # cosine similarity minimum (ketat)
